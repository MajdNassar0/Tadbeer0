...
import { User, Mail, Phone, Lock, ShieldCheck, UserCircle, Briefcase } from "lucide-react";
...
const Signup = () => {
  ...
  const signupSchema = z
    ...
    .object({
      ...
      email: z.string().min(1, "البريد الإلكتروني مطلوب").email("البريد الإلكتروني مطلوب"),
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-5">
            ...
            <motion.div variants={itemVariants} className="flex gap-4 mb-2">
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all ${formik.values.role === "User" ? "border-yellow-500 bg-yellow-50" : "border-gray-100 bg-gray-50 text-gray-400"}`}
              >
                <UserCircle size={24} />
              ...
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all ${formik.values.role === "Worker" ? "border-yellow-500 bg-yellow-50" : "border-gray-100 bg-gray-50 text-gray-400"}`}
              >
                <Briefcase size={24} />
            ...
            <motion.div variants={itemVariants} className="relative">
              ...
              <input type="tel" {...formik.getFieldProps("phone")} placeholder="رقم الجوال" dir="rtl" className={`w-full border-2 rounded-xl py-3 pr-11 pl-4 bg-gray-50/50 outline-none transition-all text-right ${formik.touched.phone && formik.errors.phone ? 'border-red-400' : 'border-gray-100 focus:border-blue-900'}`} />
            ...
            <motion.div variants={itemVariants} className="relative">
              ...
              <input type="password" {...formik.getFieldProps("confirmPassword")} placeholder="تأكيد الكلمة" className={`w-full border-2 rounded-xl py-3 pr-11 pl-4 bg-gray-50/50 outline-none transition-all ${formik.touched.confirmPassword && formik.errors.confirmPassword ? 'border-red-400' : 'border-gray-100 focus:border-blue-900'}`} />
