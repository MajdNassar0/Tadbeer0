...
import { Mail, RotateCcw, Send, ArrowRight } from "lucide-react";
...
const ForgotPassword = () => {
  ...
  // ✅ helper لتوحيد الرسائل من الباك
  const getMessage = (res) => res?.message || res?.Message || null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (payload && payload.isSuccess === false) {
        setErrorMsg(
          getMessage(payload) || "تعذّر إرسال الكود. حاول مرة أخرى"
        );
        setLoading(false);
        return;
      }
  ...
  return (
    <div
      className="min-h-screen flex flex-col items-center justify-center bg-[#f0f2f5] p-4 font-sans relative"
      dir="rtl"
    >
      {/* 🔥 زر الرجوع */}
      ...
      >
        {/* Header */}
        <div className="bg-[#001e3c] py-10 px-6 text-center text-white">
        ...
        <div className="p-8">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#f8f9fa] border-2 border-transparent rounded-lg py-3 px-4 pr-12 text-right outline-none focus:border-blue-900 focus:bg-white focus:ring-4 focus:ring-blue-50/50 transition-all duration-500"
                />
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-yellow-500 hover:bg-yellow-600 text-white py-3.5 rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg shadow-yellow-100 transition-all disabled:opacity-70"
            >
