  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov ? "0 8px 20px -4px rgba(249,115,22,0.18)" : "0 1px 4px rgba(0,0,0,0.05)",
        cursor: "pointer", transition: "all 0.18s",
        transform: hov ? "translateY(-2px)" : "none",
      }}
    >
      {service.iconUrl ? (
        <img src={getImageUrl(service.iconUrl)} alt={service.name} style={{ width: 44, height: 44, borderRadius: 10, objectFit: "cover", flexShrink: 0 }} />
      ) : (
        <div style={{ width: 44, height: 44, borderRadius: 10, background: THEME.primaryLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>🔧</div>
      )}
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontWeight: 700, fontSize: 14, margin: 0, color: hov ? THEME.primary : THEME.textMain, transition: "color 0.15s" }}>{service.name}</p>
        {service.description && <p style={{ fontSize: 12, color: THEME.textMuted, margin: "3px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{service.description}</p>}
      </div>
      <span style={{ color: THEME.primary, opacity: hov ? 1 : 0.3, transition: "opacity 0.15s", flexShrink: 0, fontSize: 18 }}>←</span>
    </div>
  );
};
