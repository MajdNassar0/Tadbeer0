...
// ── مكون نصف الدائرة (Gauge) الجديد ──
const SingleGauge = ({ value, label, color, max = 100 }) => {
  const radius = 35;
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="text-center -mt-4">
        <p className="text-base font-black text-gray-800 tabular-nums">
          {max === 5 ? value : `%${value}`}
        </p>
        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">{label}</p>
      </div>
    </div>
  );
};
...
const ACTION_TO_STATUS = { accept: "Confirmed", cancel: "Cancelled", complete: "Completed" };
...
const Dashboard = () => {
  ...
  const updateStatus = async (bookingId, action) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.patch(`${API_BASE}/Worker/Bookings/${bookingId}/${action}`, {}, {
  ...
  const avgRating = reviews?.length ? (reviews.reduce((s, r) => s + (r.rate ?? 0), 0) / reviews.length).toFixed(1) : null;
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6" dir="rtl">
      ...
      <div className="flex-1 space-y-6 min-w-0">
        ...
        {/* ── Stats Container (Grid on Right, Gauges on Left) ── */}
        <div className="flex flex-col xl:flex-row gap-4">
          ...
          <div className="grid grid-cols-2 flex-1 gap-3">
            {[
              ...
              <div key={i} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between hover:shadow-sm transition-shadow">
                <div>
                  <p className="text-[10px] text-gray-400 mb-0.5">{s.label}</p>
                  <h4 className={`text-xl font-bold text-gray-800 ${bookings === undefined ? "animate-pulse text-gray-200" : ""}`}>
          ...
          {/* Gauges Card (Left Side) */}
          <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex justify-around items-center gap-6 min-w-[280px]">
            <SingleGauge 
              value={Math.min(((completed / 50) * 100), 100).toFixed(0)} 
              label="معدل الإنجاز" 
              color="#22c55e" 
            />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-gray-50 flex justify-between items-center">
            <h3 className="text-sm font-medium text-gray-800">آخر الحجوزات</h3>
            {bookings !== undefined && <span className="text-xs text-gray-400">({total})</span>}
          </div>
          ...
          {bookings === undefined ? <Spinner /> : bookings.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-2 text-gray-300">
              ...
              <p className="text-sm">لا توجد حجوزات حالياً</p>
            </div>
          ) : (
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-gray-50/60 transition-colors">
                        ...
                        <td className="py-4 px-4 text-center">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-medium ${info.cls}`}>{info.label}</span>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center justify-center gap-1.5">
                            {sKey === "pending" && (
                              <>
                                <button onClick={() => updateStatus(b.id, "accept")} disabled={!!isAct} className="p-1.5 bg-green-50 text-green-600 rounded-lg disabled:opacity-50 transition hover:bg-green-100">
                                  {isAct === "accept" ? <Loader2 size={12} className="animate-spin" /> : <Check size={14} />}
                                </button>
                                <button onClick={() => updateStatus(b.id, "cancel")} disabled={!!isAct} className="p-1.5 bg-red-50 text-red-500 rounded-lg disabled:opacity-50 transition hover:bg-red-100">
                                  {isAct === "cancel" ? <Loader2 size={12} className="animate-spin" /> : <X size={14} />}
                                </button>
                              </>
                            )}
                            {(sKey === "confirmed" || sKey === "inprogress") && (
                              <button onClick={() => updateStatus(b.id, "complete")} disabled={!!isAct} className="px-3 py-1.5 bg-[#001F3F] text-white rounded-lg text-[10px] disabled:opacity-50">
                                {isAct === "complete" ? <Loader2 size={10} className="animate-spin mr-1" /> : <Check size={10} className="ml-1 inline" />} إتمام
                              </button>
                            )}
      ...
      <div className="w-full lg:w-72 space-y-5 flex-shrink-0">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
          ...
          {bookings === undefined ? <Spinner /> : upcoming.length === 0 ? (
            <div className="flex flex-col items-center py-6 gap-2 text-gray-300">
              <Clock size={20} />
              <p className="text-xs">لا توجد حجوزات قادمة</p>
            </div>
          ) : (
                ...
                return (
                  <div key={i} className="p-3 bg-gray-50 rounded-xl space-y-1">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-medium text-gray-800 truncate ml-2">{b.userName || "عميل"}</p>
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${info.cls}`}>{info.label}</span>
                    </div>
                    <p className="text-[10px] text-gray-400">
                      {DAYS_AR[b.workingDay?.toLowerCase()] ?? b.workingDay} · {b.bookingDate ? new Date(b.bookingDate).toLocaleDateString("ar-EG") : ""}
                    </p>
                    <p className="text-[10px] text-gray-400">{b.startTime?.slice(0,5)} – {b.endTime?.slice(0,5)}</p>
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="relative space-y-5 before:absolute before:right-4 before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-100">
            {bookings === undefined ? <Spinner /> : bookings.length === 0 ? (
              <p className="text-xs text-gray-300 text-center py-4">لا يوجد نشاط</p>
            ) : bookings.slice(0, 4).map((b, i) => (
              <div key={i} className="relative pr-9">
                <div className="absolute right-2 top-0.5 bg-white p-0.5 z-10 text-orange-400">
                  <CheckCircle2 size={14} />
                </div>
                <p className="text-[11px] font-medium text-gray-800 leading-tight">حجز من {b.userName || "عميل"}</p>
                <p className="text-[10px] text-gray-400 mt-0.5">
                  {b.bookingDate ? new Date(b.bookingDate).toLocaleDateString("ar-EG") : "—"} · 
                  <span className={`font-medium ${statusInfo(b.status).cls.split(" ")[1]}`}>{statusInfo(b.status).label}</span>
                </p>
              </div>
            ))}
          </div>
